...
@WebServlet("/RegisterProjectRoomDetails")
public class RegisterProjectRoomDetails extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ob.setProjectId(request.getParameter("selectedProjectIdforRoom"));
...
//		ob.setSamplesGiven(request.getParameter("samplesGiven"));
		...
		ob.setRoomCreatedBy((String)session.getAttribute("userId"));
		ob.setRoomUpdatedBy((String)session.getAttribute("userId"));
		
		if (ob.new_ProjectRoom()) {
			out.println("1");
		} else {
			out.println("0");
		}
